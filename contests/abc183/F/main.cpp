...
int main() {
  ...
  VI T(Q), A(Q), B(Q); REP(i, 0, Q) cin >> T[i] >> A[i] >> B[i], A[i]--, B[i]--;

  vector<unordered_map<int, int>> cnt(N);
  REP(i, 0, N) cnt[i][C[i]] = 1;
  UnionFind uf(N + 1);


  REP(i, 0, Q) {
    if (T[i] == 1) {
      int ra = uf.find(A[i]), rb = uf.find(B[i]);

      if (ra != rb) {

        if (uf.size(ra) < uf.size(rb)) swap(ra, rb);
        for (auto c : cnt[rb])
          cnt[ra][c.first] += c.second;

        uf.unite(A[i], B[i]);
      }
    }
    else {
      cout << cnt[uf.find(A[i])][B[i]] << endl;
    }
  }
